...
- `src/Main.res` — the smoke suite (`npm start`): 227 checks that
...
- **The poset round** — the context-model generalisation (linear
  ...
  cross of non-top-level / non-list axes, and the cell-set remainder
  of the partial collect. Fibered products (a product collected
...
- **The partial collect's cell-set lattice** — the design's HTTP
  ...
  computation at each depth. A merged value can be read from inside
  one of its cells by a chain that opened that cell some other way,
  and values on *overlapping* cell sets combine at their meet
  (`{A,B}` and `{B,C}` at `{B}`) — subject to the same discipline a
  product combine gets: the meet must be a set the program
  constructed. What is left: a merged flow used as a leading join
  level (a partial collect *fed* by another partial's merged flow).
