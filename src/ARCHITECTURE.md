...
register over the **whole product** stays rejected, and now **by the rule
the doc names** — Check's `order-demand`, reading the product's `Surplus`
class off `OrderDemand`'s kinds table (Main 16 unit-tests the table, Main
15m is the forcing program; the indirect `time-travel` witness from the
step's combine still fires alongside, since a Cross output port is not yet
an axis set in the poset). And the
**completion of a
...
8. **The poset round** (deferred together, because they are one
   ...
   **under-covered** consumer splits into two shapes. Where **no
   ...
   the WHOLE product, is now DONE: `OrderDemand.res` went live — the kinds
   table cashed as `orderOf`, a structural provenance walk over the flow
   constructors — and `Check.checkOrderDemand` reads it, so the grid is
   witnessed as `order-demand` ("has SURPLUS order … fold one axis, or Join
   first") rather than caught indirectly by its step's combine (Main 15m;
   Main 16 unit-tests the table, including the rows that must NOT fire — a
   register over a flattened or a filtered sequence). The indirect
   `time-travel` witness still fires alongside and evaporates when a Cross
   output port becomes an axis set in the poset. The commutative-monoid
   exception ("reduce over the whole product IS well-defined iff the
   operator commutes") is deliberately not implemented: it is discharged by
   the catalog row's commutativity flag, which `CollectFamily.res` stages
   but nothing mints yet. `instantiate` was
...
stubs naming the design doc that specifies each body. With one
exception (below) nothing in them runs or is reachable from the
pipeline; each stays staged until its emitter/check round, so the live
matches in `Program.res`/`Codegen.res` don't grow arms ahead of
behaviour. The gate for getting a stub was "has a worked representation
in the record" — adopted **or** exploration; a mere thought gets a
comment, not a module.

The first stub to go live is **`OrderDemand.res`**, and its shape is
the template for the rest: the module keeps the *table* (the kinds
lookup, pure, dependent only on `Program`), and the *rule* that reads
it moves into `Check.res`, where the witness type lives — a dependency
direction, not a change of plan. Its `failwith` bodies became a real
structural walk; what stays commentary (the `hold` identification, the
rows for unrepresented kinds) is what has no code to be yet.
...
| `OrderDemand.res` | **LIVE** (the kinds table; the rule is `Check.checkOrderDemand`) | The owned-order criterion as a five-way `orderClass` (owned / none / incidental / degenerate / surplus) and order provenance (inherited / minted / ambient), both now *cashed as a lookup* — `orderOf` reads each flow constructor's own statement about its output's order (list walk owned; option / case alt / partial-merged degenerate; join = the worse of its operands; commute inherits per port; cross surplus), so Check's rule is a provenance walk with no analysis. Rows for unrepresented kinds land with their species. The hold identification stays a recorded note. The product (surplus) cell is left to `product-linearization-design.md` — the rule rejects a whole-product register, it does not pick an axis for it. |
