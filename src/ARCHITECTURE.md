...
What compiles today: **stream flows** (the pulled-on-demand axis — `open stream` beside `open list`, one bare level, collected back into a stream; see "Stream flows" below), the value fragment (including **computed
...
10. **Stream flows** (`implementation-strategy.md` phase 5;
   ...
   *Step 2, the single-output stream flow* (`Codegen.emitStreamCollect`).
   ...
   Still ahead: **step 3, the sequence commute** — the motivating operation,
   `stream<Option<X>>` to `Option<stream<X>>` short-circuiting at the first
   None, which is the third runtime move (abandon-the-rest) the primitive
   already supports and an output construction that is not yet written; the
...
|---|---|---|
| `Stream.res` | compile strategy **committed** (Shape C, per-node memoised streams); **steps 1-2 LANDED**, so this is no longer a pure stub | What LANDED and where (`Program.flowKind`'s `Stream` row, `Runtime.streamPreludeStmts`, `Annotate`'s `StreamCell`, `Codegen.emitStreamCollect`), and what is still staged: the two commute *operations* (`SequenceCommute` vs `TransposeCommute` — settled to be distinct) with `emitStreamCommute` as step 3's stub; the stack-discipline check; the `zip` primitive (universal under Shape C, unreached under step 2's one-fold-per-collect); Shape C PROPER, i.e. per-node cells and the cross-collect sharing they buy; the source-opener riders (`SelfOpen`, `PullSource`). The consumer-set lattice stays deferred-not-rejected. |
