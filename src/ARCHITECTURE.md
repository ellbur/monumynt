...
disjointness demand both read off cell sets), **nested cell merges**
...
9. **Partial collect** — DONE for the direct slice (`Codegen.res`,
   ...
   **A partial collect built over another partial's merged flow followed**,
   ...
   Still open: a partial collect *fed* by a merged flow as a LEADING join
   level (`emitPartialCollect` still `Todo`s on a leading `PartialLevel`);
   a merged value reached by a node outside any emitter that binds it
   (`compileValue`'s `CasePartial` arm `failwith`s); and partial OVERLAP at
   the meet. Then per
