...
15k, both fiberings, hand-computed with a non-commutative operator). A
...
8. **The poset round** (deferred together, because they are one
   ...
   **under-covered** consumer splits into two shapes. Where **no
   ...
   names. What is still owed is the doc's step 1, the **"no order" witness**
   for a register over the WHOLE product: today that program is rejected
   through its step's combine (a Cross output port is not yet an axis set
   in the poset), not by the rule (Main 15m). `instantiate` was
   ...
   fiber may report just `[X]` while the chain holds `[Y, X]`. Still deferred (the context-model
