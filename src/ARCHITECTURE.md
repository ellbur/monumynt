...
9. **Partial collect** — DONE for the direct slice (`Codegen.res`,
   ...
   **The last cell-set gap — a partial level that is not innermost —
   ...
   The partial collect's cell-set row is closed. What remains of the
   cell-set world lives on OTHER rows and is listed there: the **register**
   over a partial merged driving flow has since landed too (item 6(d) —
   `emitRegister` assembles through this very `buildChain`), leaving the
   **partial running view** — about a `prev` and a loop skeleton, not about
   cells. Then per
