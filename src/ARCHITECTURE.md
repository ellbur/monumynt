...
What compiles today: **stream flows** (the pulled-on-demand axis — `open stream` beside `open list`, one bare level, collected back into a stream — the **sequence commute** over one, `stream<option<X>>` to `option<stream<X>>` short-circuiting at the first absence, which is the operation the stream layer exists for, and a **register over a stream**, `final` folding the whole source as a redirect chain with the accumulator on the side; see "Stream flows" below), the value fragment (including **computed
...
(Main 10f). A sibling may be a **filtered axis the author drew** — the
span then names both of the axis's layers, and the drawn chain claims
them as one axis (Main 10g, completing to the hand-drawn 15r5) — while an
UNDRAWN one stays a witness, completion not being allowed to mint the
join that would make it an axis (Main 10h).
A **product opened inside an enclosing loop** compiles the
...
product has no "between". The **sequence commute** — the other operation
the one word names, the directed swap with its short-circuit — now
compiles over a STREAM outer layer (see "Stream flows" and Main 16f);
over an EAGER nesting it stays representable-but-not-compilable, which
is the design holding rather than a gap ("list flows can't host commute
cleanly without becoming linear, and stream flows are the right place
for it"), and its standalone `commute out of` form is authorable in text
either way — Main 15c. Also representable-but-not-compilable
(prints, checks, and round-trips through the text surface):
...
- **Codegen state is a mutable map scoped to one invocation behind a
  ...
  (annotations, program).
- **The runtime lives in `Runtime.res`** as the emitted prelude, and is
  **LAYERED**: the eager fragment's three lazy helpers are unconditional,
  the stream cells ride along only when the node set contains a stream
  flow. Streams made the prelude non-trivial, exactly as
  compile-strategy open q.3 predicted, and layering is the smallest
  answer that keeps the promise the question was really about — a
  program should not carry runtime it does not use, and adding a flow
  kind should not change the generated JS of programs that don't use it.
  Real ES-module packaging is still the target and is not prejudged:
  the layers are already the modules it would export. Meanwhile Pipeline
  wraps one IIFE per output, each carrying the whole module's statements
  (unused lazies never run).
- **A stream is a VALUE; the stream cell is a second cell type, not a
  replacement.** `__lazy__` answers "compute at most once"; `Delayed`
  answers that *and* "my result may be another cell" — the redirect a
  become-the-rest step produces. Keeping them apart is what lets the
  stream layer be additive: a stream collect is still an ordinary
  `__lazy__` binding, and every existing emitter can hold a stream
  without knowing it is one. The alternative (one cell type for both)
  would have made every eager force pay for the redirect walk.
...
8. **The poset round** (deferred together, because they are one
   ...
   **`Complete` then followed over the same axis** (Main 10g/10h), which
   ...
   one. The blocker was that "the frontier walk resolves an axis key to a
   list uncollect's flow, and a filtered axis's flow is a Join the author
   may not even have drawn". Both halves of that are real, and they are
   different problems. The resolution half is arithmetic: a combine borne
   on `join(list, case-alt)` names BOTH layers in its span, so a span has
   three keys for two axes, and `resolveAxes` lets a DRAWN chain axis
   claim all of its layers at once (only when the span names them all — a
   value that reaches just the leading list is borne on the list, not on
   the filtered axis), with every leftover key still having to be a list
   open. `frontier` moved from keys to AXES for the same reason: an axis
   determines another when any of its LAYERS is among that one's sources,
   which is the rule it always was, stated once per axis rather than once
   per key. The undrawn half is not a gap at all but the design holding:
   completion inserts only operators whose value-level shadow is the
   identity, and a join changes firing structure, which is meaning the
   author must draw — so an undrawn filtered axis stays a `time-travel`
   witness whose remedy is the drawn chain (Main 10h), exactly as a
   non-list axis does. `axisChainOk` moved to `Context` in the same
   change, since two passes now ask the same question of a flow.
   ...
   **Completion inserts a Cross** (Cross's "smallest first step" 3) is
   ...
   `harvest`/`solve`/`realise`; Main tests 10, 10c, 10g): the k≥2 lists
...
9. **Partial collect** — DONE for the direct slice (`Codegen.res`,
   ...
   **The last cell-set gap — a partial level that is not innermost —
   ...
   cells, and every walk that handles one cell handles k by recursing.
10. **Stream flows** (`implementation-strategy.md` phase 5;
   `lazy-stream-placement-design.md`'s own staged order). **Steps 1 and 2
   are DONE**, and the shape of the round is worth recording because the
   prediction held: "each a new species in `Annotate` + cells in
   `Runtime.res` + an emitter, not a restructuring" is exactly what it
   was, and nothing in the eager pipeline had to learn what a stream is.

   *Step 1, the runtime primitives* (`Runtime.streamPreludeStmts`). The
   prototype's cells, synchronous — no promise, no `tick()`. Stripping
   `tick()` is what CREATES the doc's stack hazard, so both of its hard
   requirements landed in the primitive rather than in any stage:
   `__forceD__` follows redirects in a `while` loop (iterative force) and
   rewrites every visited cell to the final result on the way out (path
   compression). A cell is `{s, v, t, r}` with the three-state cache plus a
   REDIRECT state; the in-progress state is not decoration but the place a
   self-referential stream (the doc's open question 1) would be diagnosed.
   The stream is the prototype's two constructors and no more — `SFail` /
   a tagged end stays a settled rejection. Exercised at 200k consecutive
   become-the-rest steps without overflowing.

   *The packaging question* (compile-strategy open q.3) became real here,
   as the doc said it would, and got its cheapest honest answer: the
   prelude is still inline per output but LAYERED, and `Codegen.codegen`
   picks the layers off the node set — so an eager program's generated JS
   is byte-for-byte unchanged and a stream program pays for the stream
   cells alone. An imported runtime module remains the target; the layers
   are already its modules.

   *Step 2, the single-output stream flow* (`Codegen.emitStreamCollect`).
   `emitIterCollect` with the loop taken out. The collect is placed by
   `Context.valueContext`; the element is pre-memoised at the body context;
   statements addressed to that context are claimed into the fold step and
   everything shallower floats out. Those three moves are unchanged, and
   they are what make loop-invariant hoisting become **pull-invariant**
   hoisting with no code written for it (Main 16c: a value the per-element
   computation uses but does not vary with is computed once across three
   pulls, and still lazily). What differs is only the assembled shape — a
   `__zipStream__` fold, one output cell per source element, the branch
   value computed inside the step. The eager wrapper stays, because a
   stream is a value: the collect is a `__lazy__` binding whose forced
   value is the stream, and forcing it computes the head and no more
   (which is what `SCons(value, Delayed<tail>)` says). Routing is on
   `Annotate.speciesOf` — the annotation, not the kind.

   Two things fell out rather than being arranged. **Multi-output works by
   construction** (Main 16d), which is why the older plan's steps 4-5 are an
   optimisation pass and not a prerequisite. And a **stream flow nested
   inside an eager list flow** needed nothing at all (Main 16e): the fold
   lands inside the enclosing for-of by let-floating, and the outer collect
   gathers a list of streams. "Level identification", listed as the piece
   that is not an optimisation and must land with the first emitter, was
   already `Context.valueContext`.

   *Step 3, the SEQUENCE COMMUTE* (`Codegen.emitSequenceCommute`, routed by
   `matchSequenceCommute`; Main 16f) — the motivating operation,
   `stream<option<X>>` to `option<stream<X>>` short-circuiting at the first
   absence. The doc's claim that commute is "per-close output construction and
   nothing else" is what made it cheap: the chain feeding it is placed exactly
   as a plain stream collect's is, so the emitter is `emitStreamCollect` with a
   different assembled shape and NO new context machinery — the collect's
   exterior, the element pre-memoisation, and the statement partition are the
   same three moves, with one more level (the option's guard) in the middle.
   What the round decided rather than inherited is the fold's shape, and both
   halves are the design speaking. The fold uses two of the runtime's three
   moves and **not the third**: it never emits a cons, because a commuted
   collect answers ONE question about the whole stream and cannot hand out a
   cell before it knows the answer ("commute is exactly the place where laziness
   has to give"). Each firing BECOMES THE REST, accumulating its value on the
   side — the doc's own "it accumulates Some values" — and an absent option
   ABANDONS THE REST, returning a terminal without ever forcing the tail, so the
   short-circuit is structural and cells past the first absence are never
   pulled. And becoming-the-rest at every firing is what keeps the walk
   ITERATIVE: a fold that inspected the tail's result to build a cons would
   force the tail inside its own thunk and cost a stack frame per element, where
   a redirect chain is followed by `__forceD__`'s `while` loop — so the
   accumulator is not a shortcut around the fold, it is what lets the fold stay
   a redirect chain (Main 16f pins 50k firings). The `Some` payload is a stream
   built from the accumulated values ("it could equally be a strict list … a
   stream is the more uniform choice") and `None` is `undefined`, the same
   spelling every option-shaped value here has. Scope: the ex-outer layer must
   be a bare STREAM open, and the eager twin one word away (`open list`) still
   declines — not a gap but the design holding, "list flows can't host commute
   cleanly without becoming linear, and stream flows are the right place for it"
   (Main 15c is that program). The two swapped flows are collected SEPARATELY
   (`~c.inner` then `~c.outer`), which is the design's "closed separately"
   spelling — "the full commuted close plus an immediate re-open of the
   still-open layer" — so the pair of collects is one unit to emit; the STACKED
   stages (`Commuted(Joined)`, `Commuted(Commuted)`) and the rejection rule for
   an ill-formed stack (`Stream.checkStreamStack`) are the next width.

   *A register over a stream* (`Codegen.emitStreamRegister`; Main 16h) landed
   next, and it was the two existing pieces meeting rather than a new one.
   The check half was already done — `OrderDemand` reads a stream's order as
   OWNED, since its firings are the source's in the source's order and pull
   PACE is not order — and the emitter is the register's fold in the
   sequence commute's shape. `final` is ONE answer about the whole stream,
   so the fold never emits a cons: each firing BECOMES THE REST, advancing
   the accumulator on the side, with `prev` bound per firing ahead of the
   step's own bindings exactly as the eager payload binds it — which is also
   what keeps the walk a redirect chain `__forceD__` follows iteratively
   (Main 16h pins 50k firings, the empty-source init, and one step per
   firing). Forcing `final` walks the whole source, which is not laziness
   lost but what a fold means. Routing is `streamOpenOf` on the DRIVING flow
   inside `emitRegister` — the write half's species stays `RegisterWrite`,
   because what is stream-shaped is the flow the register names, and
   `streamOpenOf` is the same structural question Annotate answers to assign
   the collect species — and a stream layer STACKED into an eager driving
   chain still falls through to `spine`'s named gap. Placement needed
   nothing: the exterior, the element pre-memoisation, and the statement
   partition are `emitStreamCollect`'s three moves, so pull-invariant
   hoisting holds inside the fold too.

   Still ahead: the
   **flatten** `join(stream, stream)` and stream-in-stream nesting, which
   `Codegen.spine` declines with a named gap rather than compiling a pull
   chain as a loop (an eager level is a `for-of` that runs the whole source,
   and running a stream's whole source is the one thing a stream exists not
   to do); the **running view over a stream** — a sibling stream collect
   reading `prev`, which stays a clean `Todo` (Main 16h pins the decline):
   it is not `final`, since a scan CAN hand out cells as it goes, so its
   fold cons-es — a different assembled shape, the stream twin of
   `emitRunningCollect`; and **Shape C proper**, one memoised cell
   per NODE rather than one fold per collect, which is what buys back the
   cross-consumer sharing the eager model gives up. That last one is a
   placement change inside one emitter, not a semantic one, and it is what
   brings the `zip` primitive into use.

   Then per
   `implementation-strategy.md`: async/incremental — each a new
