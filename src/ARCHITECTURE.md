...
`{A} ⊆ {A, B}`, once per arm, evaluated once per firing),
...
9. **Partial collect** — DONE for the direct slice (`Codegen.res`,
   ...
   **containment theorem** stated as a step-availability relation: a bundle
   ...
   partial collect's value threading is for". Still `Todo`, the cell-set
   remainder proper: a leading `PartialLevel` (a partial feeding a
   partial — the merged-flow-into-merged-flow shape); a merged value
   reached by a node OUTSIDE its own emitter (`compileValue`'s `CasePartial`
   arm still `failwith`s, since only `emitPartialCollect` pre-memoises the
   merged value per arm — resolving it generally means reading the
   containment operationally, "the merged value inside cell A *is* A's
   branch value"); the covering **cell-set collect** (branches keyed by
   disjoint covering cell sets rather than by single alts — the HTTP
   example's final collect, which needs `classifyCollect`'s coverage read
   off cell sets); and partial OVERLAP at the meet. Then per
