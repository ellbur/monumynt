...
9. **Partial collect** — DONE for the direct slice (`Codegen.res`,
   ...
   **The last cell-set gap — a partial level that is not innermost —
   ...
   cell-set world lives on OTHER rows and is listed there: a **register**
   over a partial merged driving flow, and a **partial running view** —
   both about a `prev` and a loop skeleton, not about cells. Then per
