...
disjointness demand both read off cell sets),
...
9. **Partial collect** — DONE for the direct slice (`Codegen.res`,
   ...
   **The covering CELL-SET collect landed next**, which is the design's HTTP
   ...
   Still `Todo`, the cell-set remainder proper: a PARTIAL collect built over
   another partial's merged flow — `emitPartialCollect` reads each arm's
   cell off its branch's flow PORT, which a merged flow does not carry, so
   it declines with a clean `Todo` (Main 7n; the representation and the
   coverage classification already handle the shape, only the emitter does
   not). Also still open: a merged value reached by a node outside any
   emitter that binds it (`compileValue`'s `CasePartial` arm `failwith`s),
   and partial OVERLAP at the meet. Then per
