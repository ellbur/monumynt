...
What compiles today: **stream flows** (the pulled-on-demand axis — `open stream` beside `open list`, one bare level, collected back into a stream — and the **sequence commute** over one, `stream<option<X>>` to `option<stream<X>>` short-circuiting at the first absence, which is the operation the stream layer exists for; see "Stream flows" below), the value fragment (including **computed
...
10. **Stream flows** (`implementation-strategy.md` phase 5;
   ...
   **flatten** `join(stream, stream)` and stream-in-stream nesting, which
   ...
   to do); a **register over a stream**, where the half that is done is the
   check — `OrderDemand` reads a stream's order as OWNED, since its firings
   are the source's in the source's order and pull PACE is not order — so
   only the emitter is missing; and **Shape C proper**, one memoised cell
