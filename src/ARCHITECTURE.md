...
15r3) — as does an operand pair that repeats an axis (Main 15r4). What
does NOT yet compile is a fibered read over a filtered axis (Main 15r5,
a clean `Todo`): an axis is HELD by being traversed by index, and the
kept firings of a chain are not an indexed loop. An under-covered n-ary consumer, where no constructed product
...
8. **The poset round** (deferred together, because they are one
   ...
   **A FILTERED axis is now crossable** — an axis may be a **join chain**
   ...
   coordinates by construction. Still deferred here: a **fibered** read
   over a filtered axis (Main 15r5 — holding an axis means traversing it
   by index, and the kept firings of a chain are not an indexed loop), and
   `Complete` inserting a Cross over one (the frontier walk resolves an
