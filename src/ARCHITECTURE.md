...
8. **The poset round** (deferred together, because they are one
   ...
   **The NON-TOP-LEVEL product is DONE** — the per-group cartesian product,
   ...
   kinds' own rounds) and of axes that are **join chains** rather than
   single opens (a filtered axis is crossable by the doc's own reckoning —
   "rectangular, just smaller" — but its identity is not a single axis key
   in the flow-variable model, which is the poset-VALUED report's problem,
   not a matcher's); Cross output ports for the barrier
