...
> repo.** The algorithm operates on the semantics-free graph of
> `graph-representation.md` and is unaffected by the language-side
> supersessions; only examples that mention rails or tap nodes use retired
> vocabulary (the current construct is Delay — see
