...
*Exploration round, 2026-07-09, written directly against the
real-loop surveys' breadth set and the tough-use-cases inventory.
**Nothing here is adopted.** The constructs this document works out —
split-when as a segmenting flow operation, and the running view as a
derived port of a collect — are proposals with leanings, prepared for
the design conversation; per the sampling method's own rule, evidence
reweights but decisions stay in the conversations. What this round
contributes: the demand assembled in one place, a reframing that
dissolves "advance how far" into boundary placement, one candidate
shape worked against the record's constraints, the running-view
question answered with machinery the record already contains, the
three ownerless breadth-set members worked end-to-end against the
proposals, and three dead ends recorded so they are not re-proposed.*
...
designed owner, and the survey's own words locate "where the
language's breadth risk currently concentrates" exactly here
(`real-loop-survey.md`, "the 80/20 counterweight"). The end-when round
deliberately bounded this cluster out ("the take question stays with
the decision-driven family round" — `end-when-design.md`, "The
decision-driven family: scope bounded"). This is that round.
...
- **The wrap loop** (breadth item 1; survey 1, textwrap). The richest
  ...
  emitted yet; the truncation path *rewriting* `lines[-1]` and
  breaking). The survey's verdict: "nothing in the current or
  candidate inventory covers this loop as one reading."
- **The tokenizer-substituter** (breadth item 2; survey 2, textual
  css/parse.py). Pull-based consumption at a variable rate — nested
  sub-loops consume more tokens depending on what was seen — plus a
  keyed accumulator written *and read back* during the walk
  (definitions collected, then substituted into later references).
- **The DP table fill** (breadth item 4; survey 2, mpmath gammazeta).
  A collect whose per-element body indexes into the collect's own
  earlier output — scan-with-full-history, "stronger than the
  running-view question and stronger than window," and "the bread of
  numerics."
- **The running view, three guises** (survey 2, finding 2.5): the
  ...
  filed "somewhere between multi-close (what may be observed of an
  iteration while it runs) and the register designs (what may be
  carried)," owned by neither.
- **Protocol framing** (`tough-use-cases-design.md`, use case 5):
  "read exactly N more bytes, where N came from the header" —
  decision-driven consumption from the datagram side; the third
  arrival that made the family load-bearing.
- **The decision-driven family's unworked members**
  (`tough-use-cases-design.md`, inventory item 4 and open question 5):
  the ordered/decision-driven merge has a worked sketch (two flows,
  per-heads chooser); *data-dependent take* — "advance how far" — has
  only its name. Question 5 asks whether the family is one primitive
  (a chooser over N heads) or a small family; the end-when round
  answered the advance-or-stop half (separate constructs, shared
  verdict vocabulary) and left the advance-how-far half here.
- **Small everyday sightings**: skip-while (survey 1, js 7 — a
  predicate cursor whose "loop condition" is a data predicate),
  chunking (survey 2, three 1), run-length-encoding-shaped adjacency
  grouping generally.

One observation organises the whole round: in the wrap loop, the two
halves of this cluster are *interlocked* — the boundary decision
("does this chunk fit?") depends on the width, which depends on the
indent, which depends on the running view (has any line been emitted
yet?). This is why the open-problems row names them together, and why
this round works them together.
...
"does this firing satisfy P": a case split producing alt flows.
Filter consumes such an alt (join); end-when consumes such an alt
(stop). Higher-order predicate arguments are rejected
...
variable-rate consumer of a flow must obey the same rule — other
collects of the same flow see the full, flat walk.
...
(`async-flow-design.md`). End-when writes a terminator on its derived
flow. Anything that ends a *segment* early has this machinery
...
(`partial-collect-design.md`). End-when's stop operand is one; a
boundary operand will be one too.

**The stream runtime has three moves** — emit-and-continue,
become-the-rest, abandon-the-rest
(`lazy-stream-placement-design.md`, "The skip mechanism"). A
construct that compiles to these rather than demanding a fourth is at
the right altitude.
...
precedent. A register on a nested flow reinitialises per firing of
the outer flow — nested loops already mean this.
...
a sum"). This round gives that machinery its second — and first
*everyday* — client.
...
The family's name for this gap — *data-dependent take*, "advance how
far" — inherits imperative furniture: a cursor, and a count of how
far to move it. Look instead at what every sampled instance is
actually doing:

- textwrap consumes chunks *until the line is full* — the chunks of
  one line are a **run** of the chunk sequence, and the loop's real
  content is where one run ends and the next begins;
...
Stated the way join and end-when were stated:
...
Consequences that are theorems of the law rather than extra design:
...
- **Per-segment state costs nothing new.** A register on the inner
  flow reinitialises at each outer firing — that is just what
  registers on nested flows mean. "Reset per line," "reset per
  message" needs no reset construct; the nesting is the reset.
- **Per-segment readout is discharge.** A whole-segment collect
  ...
  per-segment case split on existing machinery, exactly parallel to
  end-when's readout.
- **The empty and total cases are unremarkable.** A boundary firing
  ...
  real programs (str.split on adjacent delimiters) and other
  programs filter them out downstream (textwrap's `if cur_line:`) —
  an ordinary filter on the outer flow, not a mode of the node.

And the same operand pattern now recurs a third time, which is worth
a table:
...
End-when needed one bit (the stopping element in or out —
`end-when-design.md`, "The stopping element"). Split-when's boundary
element has three possible destinations, and all three are everyday,
each sighted in the wild:
...
- **Starts the next segment** (the exclusive reading). The wrap
  ...
  grouping (RLE, groupby) is the same: the element that differs
  begins the new run.
- **Ends the current segment** (the inclusive reading). Framing:
  the byte that completes the message belongs to the message.
...
- **Dropped** (the delimiter reading). Splitting on separators:
  ...
  segment terminator, so "which delimiter" is not lost.

The leaning mirrors end-when's option B: this is genuinely a small,
local, enumerated choice on the node (or on the boundary wire — same
...
segment after a stop). It is configuration in the established sense,
not a construct.

One consequence to note rather than smooth over: under the
...
fit, so it goes on the next line" means, and it is what every
...
## Well-formedness: the boundary may read per-segment state, and the check transfers
...
split reads the register's state port; the boundary feeds split-when.

The cycle is productive, and by the existing rule, not a new one: the
...
grounds everything. A boundary wired from the *current* element alone
(sentinel, adjacency against a carried previous element) doesn't even
have the cycle.

This is the same reassurance end-when got from the three runtime
moves: the construct's hard case lands on a check the language
already runs.

## Prior art

The shape is heavily precedented, which is evidence for the
reframing (the way Lustre's `pre`/`->` was for the Delay):
...
- **Python** `itertools.groupby` — adjacent grouping by key change:
  ...
  partitions by key value regardless of adjacency — the two are
  different constructs in the wild too, a reassuring confusion to
  find already disambiguated by this record's vocabulary.)
- **SQL's "gaps and islands"** — sessionisation by time gap,
  islands of consecutive activity. Famous as a *hard problem*
  precisely because SQL lacks the construct and it must be built
  from window functions and running counts — a whole genre of blog
  posts is the field's testimony that the missing vocabulary hurts.
- **Parser toolkits' take-while families** — the take-while/span/
  break cluster is the one-segment special case, ubiquitous.
...
The sharpest standing challenge, drawn:
    ...
    width:    indent-dependent width (see below — the running view)
    ...
    truncate: end-when on the outer segment flow (a count register's
              alt fires at max_lines); the discharged terminator
              says truncation happened and carries what remained;
              the placeholder amendment to the last line happens
...
recorded as an open question, not designed here.
...
The predicate cursor (js 7) — skip leading matches, keep the rest —
is split-when with a boundary at the first non-match, keeping the
...
(eighteen of sixty loops; its output is a flow, not a flow of
flows); split-when is the breadth construct. A level-1 recognition
("an end-when is a split-when observed at its first segment") can
relate them in the tower without either construct wearing the
other's weight. What they must share is the boundary/stop operand
discipline and the destination bit's vocabulary — the same family-level sharing as the
...
- **which of two advances** — the decision-driven merge, still owed
  ...
  interleaving), and this round deliberately leaves it the chooser
  sketch it has.
...
- **List subject**: the lowering is the textwrap loop itself — an
  ...
  pushed before or after the check (or not at all). That the
  construct's derived lowering is verbatim the code the survey drew
  at random is the same strong-form philosophy argument the ordered
  merge made: the lowering exists, is correct, and nobody should
  read it unless they ask.
- **Stream subject**: the outer stream's pull produces a segment
  whose inner stream consumes from the shared source cursor;
  ending a segment is abandon-the-rest aimed at the segment's
  terminator — the third existing move, no fourth needed. The real
  constraint is sequencing: inner flows share one underlying
  cursor, so segments must be consumed in order, and an outer
  consumer that skips ahead forces either draining or buffering the
  skipped segment — the same retention/pull-amplification footgun
  family the stream docs already track for `Delayed`. Flagged for
  the stream rounds, not designed here.
...
the walk.** The survey filed it between multi-close and the
registers. It belongs to neither; it falls out of machinery the
record has already built, in the iteration-state round's own
vocabulary:
...
contributes to a collect and reads its running view is a cycle:
body → collect → derived register → state port → body. It is
productive by the existing rule — the read crosses the derived
...
- **Sibling reads are time travel.** The running view is readable in
  ...
  rule). Reading one loop's so-far from a sibling loop that isn't on
  the same nesting chain is the ordinary no-time-travel violation,
  caught by the ordinary check. Nothing new.
...
- **Read-by-index** (the DP table): index into the running list —
  a value operation on the port. The efficiency worry dissolves in
  ...
  (This is visible in the lowering: `s += ... zeta_values[2*k] ...`
  — the wild code *is* the compile target.)
...
Textwrap's truncation path *rewrites* `lines[-1]` mid-walk. A
mutable running view — writing into a collect's past from the body —
would be retroaction on already-emitted values, exactly what
derivation-not-retroaction forbids, and it is not needed. Worked
against the actual program: the rewrite happens *once, at
termination* — it is not a mid-walk operation at all, it is part of
the readout. The time-forward expression: end-when cuts the segment
flow at max_lines; the discharged terminator says the walk was
truncated (and carries what remained); downstream, on the collected
prefix — a plain list value in hand — the last element is amended.
Collect, discharge, amend: all existing vocabulary, all after the
walk, which is when the wild program actually knows truncation
happened too. The mid-walk appearance of the rewrite in the Python
is an artifact of writing the readout inside the loop body for lack
of anywhere else to put it.

(General rule worth recording: a "rewrite of the output-so-far" that
is conditioned on termination belongs downstream of discharge; one
conditioned on later *elements* is a one-firing lookahead, which is
window/negative-delay territory and was not drawn in either survey.
If a sample ever produces the second kind, it files there, not
here.)
...
- The iteration-state decision gains a datum: whichever candidate is
  ...
  because the running view is defined through it. This is pressure
  on the *machinery*, not a thumb on the scale between the
  candidates — both candidates can express the augment loop; what
  matters is that the derived-view story stays real.
...
- **Foundations before features.** Split-when's new content is one
  ...
  *no* construct: it is a definition over the derived-view
  machinery. Three dead ends died on paper (below).
- **Programmer's abstraction level.** "Group these into runs,"
  "the lines so far" are words in the programmer's vocabulary, and
  each gets one reading. The gaps-and-islands genre is the
...
- **Building blocks must build.** The ladders, walked:
  - *Plain collect* → **+ segmentation** (add a boundary split and
    ...
    **+ truncation** (end-when on the outer flow). The top of this
    ...
    collect with no species change at any rung, which is the
    expansion test the breadth set demands.
  - *Collect* → **+ read the so-far** (wire from the port) → **+
    read by index/key** (value ops on the same wire) → **+ the DP
    fill** (the body's inputs now include the port; nothing else
...
breadth-set items 1, 2, and 4 gain owners to be tested against (and
the breadth set's "no owner" lines gain referents); the tough doc's
open question 5 closes (end-when standalone — already leaned;
advance-how-far dissolved into boundary placement; the merge stays
the family's one chooser member, pending its round); survey finding
2.5's question gets its answer recorded (derived state port, strict
...
1. **The destination setting's final form.** Three values on the
   ...
   vs independent splits — assumed to transfer along the worked
   pattern, unverified).
...
8. **Textual spellings.** The (subject, boundary) pair, the
   ...
   textual form; that document's next round.
9. **Naming.** "Split-when" deliberately rhymes with end-when;
   "running view" vs "so-far" vs the survey's phrase. Deferred per
   tradition, with one constraint each: the segment construct's name
   should read as a flow operation yielding *nested* flow (the
   nesting is the point), and the view's name must not suggest
   mutability.

## Dead ends, recorded

**A count-valued advance verdict.** `advance(k)` — a chooser verdict
or node operand carrying how many elements to consume — was the
family's inherited framing and is rejected as the surface: the count
is the imperative encoding of a boundary (no sampled program's
*meaning* contained the number; every one contained the run), it
puts a bare integer where the record wants drawn structure, and it
loses the boundary payload that discharge gives free. Where the
data genuinely is a count (framing's N), it enters as data — a wire
into a register the boundary reads — not as the construct's verdict.
Not to be re-proposed; the boundary reframing supersedes it.

**A mutable running view.** Letting the body write into the
collect's past (`lines[-1] = ...` mid-walk) — rejected on
derivation-not-retroaction grounds: it would make one consumer's
body retroactively change what every other consumer of the same
collect sees, and it is unnecessary — the sampled rewrite is
...
**Variable-rate consumption as a collect mode.** A "collect until
full / collect n" collect-variant — fusing the boundary into the
collect node — is rejected for end-when's reason exactly: the
readout composition needs the cut upstream of an ordinary collect
(so the segment can be collected, effect-walked, or multi-closed by
independent consumers), and a fused node would re-decide the
boundary per consumer under multi-close. The cut is a flow
operation; collects stay collects.
