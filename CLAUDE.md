...
- `src/next/` — the **next-generation scaffolding** (started 2026-07-17): ports-first representation (`Program.res` — node set + outputs, binary Join, per-alt ports, Delay pair; uncollect/collect vocabulary), typed handles (`Build.res`), context paths (`Context.res`), the pipeline as typed passes (`Check`/`Complete`/`Annotate`/`Codegen`, partly stubs) with a disposable `LegacyBridge` into `Compile.res`, and the textual form's lexer/parser/resolver/printer (`Text*.res`). Read `src/next/ARCHITECTURE.md` first — it records module status, the provisional decisions, and the growth path. Smoke suite: `npm run next` (currently 25 checks). `Expr`/`Compile`/`ExprPrint`/`Main` are the previous generation, kept green as the rebuild's spec until codegen lands.
