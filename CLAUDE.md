...
- `src/Expr.res` — visual-language expressions. Three mutually recursive types: `expr` (nodes, `{id: int, kind: kind}`), `valueRef` (`ValuePort(node, portName)` — every value input position takes one; today every node has at most one value port, named `"value"`), and `flowRef` (references to flows). This is step 1 of the `plans/first-class-ports-design.md` migration; steps 2–3 (per-alt ports dissolving Branch, binary Join dissolving the wrappers) are still to come. Five node kinds in `expr`: `Lit`, `App`, `Open`, `Close`, `Branch`. Smart constructors (`lit`, `app`, `open_`, `close_`, `caseClose`, `branch_`) mint fresh ids and return a `handle` — `{node, value}` — so authoring spells `x.value` for a value port and `NodeFlow(x.node)` for a flow port (`handle` is defined outside the `type rec` group so its `value` label and `closeBranch`'s don't trip Warning 30). Three `flowRef` constructors: `NodeFlow(expr)` (the flow output port of a node — Open, Branch — useable when that node has one), `Joined(flowRef)`, `Filtered(flowRef)`. The flow-only ops `join_` and `filter_` take and return flowRef. Open's `flow` is `ListIter | CaseSplit({alts, discriminator}) | OptionIter` (OptionIter's input is itself the option: `undefined` means None; convert other encodings upstream via App). Close has `branches: array<{altName: option<string>, flow: flowRef, value: valueRef}>`; the kind is determined by the shape of `branches[0].flow` after peeling Joineds: a NodeFlow(branch) ⇒ case close; a Filtered ⇒ filter close; a NodeFlow(open ListIter) ⇒ list close; a NodeFlow(open OptionIter) ⇒ option close. Branch picks one output port from a CaseSplit Open. The `flowRef` type catches misuse syntactically (you can't try to App a flow, nor branch off a Lit).
...
- `src/next/` — the **next-generation scaffolding** (started 2026-07-17): ports-first representation (`Program.res` — node set + outputs, binary Join, per-alt ports, Delay pair; uncollect/collect vocabulary), typed handles (`Build.res`), context paths (`Context.res`), and the pipeline as typed passes (`Derive`/`Check`/`Complete`/`Annotate`/`Codegen`), plus the textual form's lexer/parser/resolver/printer (`Text*.res`). Pass 4 has **two engines**: `Codegen.res` (the rebuild — pure let-floating placement, (node, port, context) memo, thunk-tagged contexts; Lit/App/iter-collect emitters working, the rest raise `Codegen.Todo`) and the disposable `LegacyBridge` into `Compile.res`, which `Pipeline.compile` falls back to on `Todo`; `NextMain` differential-checks the engines against each other wherever both compile. Read `src/next/ARCHITECTURE.md` first — it records module status, the decisions, and the fill-in worklist. Smoke suite: `npm run next` (currently 32 checks). `Expr`/`Compile`/`ExprPrint`/`Main` are the previous generation, kept green as the rebuild's spec (each Codegen `Todo` stub names its legacy spec function) until the bridge retires.
...
- `compileExpr(ctx, e, currentBody)` returns a name. `lookupMemo` scans the id's entries for one whose body `isAncestor` of `currentBody` and reuses it; otherwise it emits fresh. The ancestor check is what stops a binding that lives inside one thunk from being referenced out of another. Value input positions hold `valueRef`s; `compileValueRef` projects to the node (per-port dispatch arrives with ports-migration step 2).
...
- **Sharing is opt-in via ReScript-level binding.** Two calls to `Expr.lit(int_(5))` produce two distinct nodes (different ids, no sharing). To share, bind the handle once and reuse its ref: `let x = lit(int_(5)); app(f, [x.value, x.value])`.
...
- **Value port vs flow port** is conceptually distinct. Since ports-migration step 1, value wires are `ValuePort(node, "value")` refs — but the compiler still operates on whole nodes (`compileValueRef` just projects), so the conflation of a node with its single value output port survives underneath. This works for Lit/App/Close; Open and Branch (whose value ports only exist inside a consuming Close's thunk) are surfaced as `failwith` when reached as a value outside one. Migration steps 2–4 (per-alt ports, binary Join, checks) are the remaining next step (staged in `plans/first-class-ports-design.md`; see README).
