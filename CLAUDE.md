...
- `src/Expr.res` — visual-language expressions. Six node kinds: `Lit`, `App`, `Open`, `Close`, `Join`, `Branch`. Every node is wrapped as `{id: int, kind: kind}`; smart constructors (`lit`, `app`, `open_`, `close_`, `caseClose`, `join_`, `branch_`) mint fresh ids from a module-local counter. Open's `flow` is `ListIter | CaseSplit({alts, discriminator})`. Close has only `branches: array<{altName: option<string>, flow: expr, value: expr}>` — no flow discriminator on the Close itself; the kind is determined by the underlying Open it consumes (a single None-branch + opener-shape flow ⇒ list close; one Some(name)-branch per alt + Branch-shape flow ⇒ case close). Branch picks one output port from a CaseSplit Open — same node serves both value and flow contexts.
